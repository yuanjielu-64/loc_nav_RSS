...
namespace Antipatrea {
    ...
    TEB_DDPPlanner::TEB_DDPPlanner() : teb_failure_count_(0), use_ddp_fallback_(false) {
        ...
        std::cout << "[INFO] " << "TEB_DDPPlanner initialized - TEB with DDP fallback");
    }
    ...
    bool TEB_DDPPlanner::handleNoMapPlanning(geometry_msgs::msg::Twist &cmd_vel) {
    ...
    bool TEB_DDPPlanner::handleNormalSpeedPlanning(geometry_msgs::msg::Twist &cmd_vel, double dt) {
        normalParameters(*robot);
        ...
        if (!success) {
            teb_failure_count_++;
            RCLCPP_WARN_THROTTLE(robot->get_logger(), *robot->get_clock(), 1000,
                "TEB planning failed (count: %d), trying DDP fallback", teb_failure_count_);
            ...
            if (!success) {
                // Both failed - stop
                RCLCPP_WARN_THROTTLE(robot->get_logger(), *robot->get_clock(), 1000, "Both TEB and DDP failed, stopping");
            ...
            if (teb_failure_count_ > 0) {
                RCLCPP_INFO_THROTTLE(robot->get_logger(), *robot->get_clock(), 1000,
                    "TEB recovered after %d failures", teb_failure_count_);
            }
    ...
    bool TEB_DDPPlanner::handleLowSpeedPlanning(geometry_msgs::msg::Twist &cmd_vel, double dt) {
        ...
        if (!success) {
            teb_failure_count_++;
            std::cerr << "[WARN] " << "TEB planning failed (low speed), trying DDP fallback");

            // Use DDP as fallback
            success = callDDPPlanner(cmd_vel, dt);

            if (!success) {
                std::cerr << "[WARN] " << "Both TEB and DDP failed, stopping");
                publishCommand(cmd_vel, 0, 0);
            }
        } else {
            teb_failure_count_ = 0;
    ...
    bool TEB_DDPPlanner::handleAbnormalPlanning(geometry_msgs::msg::Twist &cmd_vel, double dt) {
    ...
    bool TEB_DDPPlanner::callTEBPlanner(geometry_msgs::msg::Twist &cmd_vel) {
        // 1. Build global plan from local_goal
        std::vector<geometry_msgs::PoseStamped> global_plan;
        buildGlobalPlan(global_plan);

        if (global_plan.size() < 2) {
            std::cerr << "[WARN] " << "Global plan too short for TEB");
            return false;
        }
        ...
        if (!success) {
            std::cerr << "[WARN] " << "TEB optimization failed");
            return false;
        }
    ...
    bool TEB_DDPPlanner::callDDPPlanner(geometry_msgs::msg::Twist &cmd_vel, double dt) {
        ...
        std::cerr << "[WARN] " << "DDP fallback not yet implemented");
        return false;
    }
    ...
    void TEB_DDPPlanner::publishCommand(geometry_msgs::msg::Twist &cmd_vel, double linear, double angular) {
