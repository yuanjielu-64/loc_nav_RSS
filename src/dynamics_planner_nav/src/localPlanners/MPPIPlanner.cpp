...
namespace Antipatrea {
    ...
    bool MPPIPlanner::handleNoMapPlanning(geometry_msgs::msg::Twist &cmd_vel) {
    ...
    bool MPPIPlanner::handleNormalSpeedPlanning(geometry_msgs::msg::Twist &cmd_vel,
    ...
    bool MPPIPlanner::handleLowSpeedPlanning(geometry_msgs::msg::Twist &cmd_vel,
                                                std::pair<std::vector<PoseState>, bool> &best_traj, double dt) {
    ...
    void MPPIPlanner::publishCommand(geometry_msgs::msg::Twist &cmd_vel, double linear, double angular) {
    ...
    bool MPPIPlanner::mppi_planning(PoseState &state, PoseState &state_odom,
        ...
        // ============ MPPI历史控制序列初始化 ============
        // 首次运行或步数不匹配时初始化
        if (!has_previous_solution_ || u_optimal_.size() != static_cast<size_t>(nr_steps_)) {
            u_optimal_.clear();
