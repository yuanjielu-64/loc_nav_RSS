...
namespace Antipatrea {
    ...
    bool DDP::handleAbnormalPlaning(geometry_msgs::msg::Twist &cmd_vel,
                                    std::pair<std::vector<PoseState>, bool> &best_traj, double dt) {
    ...
    void DDP::publishCommand(geometry_msgs::msg::Twist &cmd_vel, double linear, double angular) const {
    ...
    double DDP::calc_obs_cost(const std::vector<PoseState> &traj, double &t) {
        ...
        for (size_t i = 0; i < traj.size() - 1; ++i) {
            ...
            for (auto &obs: obss) {
                ...
                if (use_space_cost_) {
                    if (i > static_cast<size_t>(3 * traj.size() / 4)) {
